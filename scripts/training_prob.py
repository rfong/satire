...
for type in types:
	counts[type] = util.read_counts("/mit/rfong/Private/6.864/proj/data/bag/training/count-"+type)
#	for w,c in counts[type].iteritems():
#		fout_word.write(w+' %f'%(c/sum[type])+'\n')

# P(y) forall y
#fin_type = open("/mit/rfong/Private/6.864/proj/data/bag/training/prob-type", "r")			# P(y) -- overall probability of satire categorization
...
for type in ["satire", "true"]: # for y=true,false
	fout_doc = open("/mit/rfong/Private/6.864/proj/data/bag/training/prob-doc-"+type, "w")	# P(y|x) -- probability of satire categorization given word 
